...
static bool jl_finalize_function(Function *F, Module *collector = NULL)
{
    ...
    if (m) {
        ...
        for (Module::iterator I = m->begin(), E = m->end(); I != E; ++I) {
            if (F->isDeclaration() && !F->isIntrinsic()) {
                bool modified = jl_finalize_function(&*I, collector ? collector : m.get());
                if (modified && !collector) {
                    // jl_finalize_function modifies the collector,
                    // so need to restart if this was iterating over it
                    I = m->begin();
                    E = m->end();
                }
            }
