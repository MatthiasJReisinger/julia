...
static int jl_getDylibFunctionInfo(jl_frame_t **frames, size_t pointer, int skipC)
{
    ...
    lookup_pointer(context, frames, pointer+slide, isSysImg);
    if (isSysImg && sysimg_fvars) {
        ...
        return lookup_pointer(context, frames, pointer+slide, isSysImg);
    }
}
...
int jl_getFunctionInfo(jl_frame_t **frames_out, size_t pointer, int skipC)
{
    ...
    *frames_out = frames;
    ...
    if (jl_DI_for_fptr(pointer, &symsize, &slide, NULL, &object, &context)) {
        ...
        int nf = lookup_pointer(context, frames_out, pointer+slide, 1);
...
// Without MCJIT we use the FuncInfo structure containing address maps
    ...
    uv_rwlock_rdunlock(&threadsafe);
#endif // USE_MCJIT
    return jl_getDylibFunctionInfo(frames_out, pointer, skipC);
}
