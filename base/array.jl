...
_similar_for(c::AbstractArray, itr, T, ::SizeUnknown) = similar(c, T, 0)
_similar_for(c::AbstractArray, itr, T, ::HasLength) = similar(c, T, Int(length(itr)::Integer))
_similar_for(c::AbstractArray, itr, T, ::HasShape) = similar(c, T, convert(Dims,size(itr)))
_similar_for(c, itr, T, isz) = similar(c, T)
...
_collect(cont, itr, ::HasEltype, isz::Union{HasLength,HasShape}) =
    copy!(_similar_for(cont, itr, eltype(itr), isz), itr)

function _collect(cont, itr, ::HasEltype, isz::SizeUnknown)
    a = _similar_for(cont, itr, eltype(itr), isz)
    for x in itr
        push!(a,x)
    end
    return a
...
_collect(c, itr, ::EltypeUnknown, isz::SizeUnknown) = grow_to!(_similar_for(c, itr, Union{}, isz), itr)

function _collect(c, itr, ::EltypeUnknown, isz::Union{HasLength,HasShape})
    st = start(itr)
    if done(itr,st)
        return _similar_for(c, itr, Union{}, isz)
    end
    v1, st = next(itr, st)
    collect_to_with_first!(_similar_for(c, itr, typeof(v1), isz), v1, itr, st)
end
